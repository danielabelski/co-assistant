...
| **`session.ts`** | Manages a pool of Copilot sessions for parallel message processing. `createSession(model, tools)` converts `ToolDefinition[]` into SDK `Tool[]` via `defineTool`, creates the session with `onPermissionRequest: approveAll`. Supports both blocking (`sendAndWait`) and streaming (`send` + event listeners). On every message, `applySystemContext()` loads `personality.md` (AI behaviour) and `user.md` (owner profile) fresh from disk and injects them as a `<system>` prefix — no restart required for changes. `switchModel()` and `updateTools()` close and recreate sessions because the SDK binds tools at creation time. |
